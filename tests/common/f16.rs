...
//! `tests/fixed_*_float_q_galois_f16.rs` integration tests.
//!
//! Lives outside `tests/common/mod.rs` because referencing `f16`
//! requires `#![feature(f16)]` at the test crate root, and only the
//! `*_f16.rs` test files opt in to nightly. Each f16 test file
//! pulls this in via `#[path = "common/f16.rs"] mod common_f16;`.
