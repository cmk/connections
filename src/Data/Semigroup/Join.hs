...
import safe Prelude hiding (Bounded, not)
...
instance (Ord k, Semigroup (Join a)) => Semigroup (Join (Map.Map k a)) where
...
instance (Ord k, Semigroup (Join a)) => Monoid (Join (Map.Map k a)) where
  mempty = Join Map.empty

instance (Ord k, Semigroup (Meet a)) => Semigroup (Meet (Map.Map k a)) where
  (<>) = liftA2 (Map.intersectionWith (/\))

instance (Ord k, Finite k, Monoid (Meet a)) => Monoid (Meet (Map.Map k a)) where
