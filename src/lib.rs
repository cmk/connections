...
//!
//! A Galois connection (adjoint pair `f ⊣ g`) between two preordered
//! sets, stored as two `fn` pointers plus a phantom kind tag. Adjoint
//! triples (`f ⊣ g ⊣ h`) are zero-sized markers implementing both
//! [`ConnL`](conn::ConnL) and [`ConnR`](conn::ConnR). See
...
//! on the kind-specific impl blocks, not free functions. The same
//! method names appear as default trait methods on [`ConnL`](conn::ConnL)
//! / [`ConnR`](conn::ConnR), so triple markers expose them too via the
//! blanket impl.
//!
//! - L-side: `ceil` (lower adjoint, rounds up), `upper` (upper adjoint,
//!   embedding), plus `ceil1`/`2`, `upper1`/`2` lifters.
//! - R-side: `floor` (upper adjoint, rounds down), `lower` (lower adjoint,
//!   embedding), plus `floor1`/`2`, `lower1`/`2` lifters.
...
//!   [`ConnK`](conn::ConnK) (`= ConnL + ConnR`) and re-export at the
//!   crate root.
//!
//! The L/R discipline is structural: `.floor(...)` on an L-kind Conn
//! is a compile error (the method only exists on `Conn<_, _, R>`),
//! and vice versa. Two-sided helpers similarly reject one-sided
//! operands at compile time because a one-sided `Conn` doesn't impl
//! `ConnK`. See [`conn`] for the rationale.
//!
//! ## Composition
//!
