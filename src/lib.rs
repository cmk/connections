...
//! invocation. The bare [`compose!`](crate::compose) is a forwarder
//! to `compose_l!`, mirroring the `K = L` default of [`Conn`]. Runtime
//! composition is deferred until a closure-capturing `DynConn`
//! variant lands — see `doc/design.md`.
//!
//! ## Codegen macros
//!
...
//!   implementing [`ConnL`](conn::ConnL) via default-method dispatch):
//!   `ceil`, `upper`, plus the `1` / `2` lifters.
//! - R-kind methods on [`Conn<_, _, R>`](conn::Conn) (and on
//!   [`ConnR`](conn::ConnR) implementors): `floor`, `lower`, plus the
//!   `1` / `2` lifters.
...
//!   [`ConnK`](conn::ConnK) and re-export at the crate root.
//!
//! The Haskell `Data.Connection.Cast` module distinguishes L-side
//! names (`ceil`/`upper`) from R-side names (`floor`/`lower`) via
...
/// | [`compose!`]          | forwarder to [`compose_l!`] (mirrors the `K = L` default of [`Conn`]).        |
