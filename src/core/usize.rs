...
//!
//! - [`USZEU008`] — `usize → u8`, saturating narrow (a narrowing on every
//!   target, since `usize ≥ 16 > 8` bits).
//! - [`USZEU016`] — `usize → u16`, saturating narrow (the identity iso on a
//!   16-bit target, a narrowing on 32/64-bit).
//! - [`USZEU032`] — `usize → u32`, saturating narrow (iso on 32-bit; a
//!   lossless widen on 16-bit, a narrowing on 64-bit).
//! - [`USZEU064`] — `usize → u64`, saturating embed (iso on 64-bit; a
//!   lossless widen on 16/32-bit).
//! - [`USZEU128`] — `usize → u128`, saturating embed (lossless on every real
//!   target).
//!
...
pub const USZEU008: Conn<usize, u8> = {
    fn ceil(x: usize) -> u8 {
...
pub const USZEU016: Conn<usize, u16> = {
    fn ceil(x: usize) -> u16 {
...
/// stay lawful because both clamp with `TryFrom`. Unlike [`USZEU008`] /
/// [`USZEU016`], `inner`'s widen must be fallible `usize::try_from`: `u32`
/// does not fit a 16-bit `usize`, so there is no `From<u32> for usize`.
pub const USZEU032: Conn<usize, u32> = {
    fn ceil(x: usize) -> u32 {
...
pub const USZEU064: Conn<usize, u64> = {
    fn ceil(x: usize) -> u64 {
...
/// widening, like [`USZEU064`]).
pub const USZEU128: Conn<usize, u128> = {
...
mod tests {
    ...
    #[test]
    fn uszeu008_saturates_and_fixes_up() {
        // Width-agnostic: usize::MAX ≥ 65535 > u8::MAX on every target, so
        // ceil always saturates and the fixup pins inner(u8::MAX).
        assert_eq!(USZEU008.ceil(usize::MAX), u8::MAX);
        assert_eq!(USZEU008.upper(u8::MAX), usize::MAX);
        assert_eq!(USZEU008.upper(0), 0);
        assert_eq!(USZEU008.ceil(200), 200);
        assert_eq!(USZEU008.upper(200), 200);
    }

    #[test]
    fn uszeu016_saturates_and_fixes_up() {
        // On a 16-bit target usize::MAX == u16::MAX, so ceil(usize::MAX) is
        // the iso image; on 32/64-bit it saturates. Either way == u16::MAX.
        assert_eq!(USZEU016.ceil(usize::MAX), u16::MAX);
        assert_eq!(USZEU016.upper(u16::MAX), usize::MAX);
        assert_eq!(USZEU016.upper(0), 0);
        assert_eq!(USZEU016.ceil(1_000), 1_000);
        assert_eq!(USZEU016.upper(1_000), 1_000);
    }

    #[test]
    fn uszeu032_saturates_and_fixes_up() {
        // Width-agnostic: the FINE_MAX fixup pins inner(u32::MAX) to
        // usize::MAX on every target, and small values round-trip.
        assert_eq!(USZEU032.upper(u32::MAX), usize::MAX);
        assert_eq!(USZEU032.upper(0), 0);
        assert_eq!(USZEU032.ceil(1_000), 1_000);
        assert_eq!(USZEU032.upper(1_000), 1_000);
        ...
        {
            assert_eq!(USZEU032.ceil(usize::MAX), u32::MAX);
            assert_eq!(USZEU032.ceil(u32::MAX as usize + 1), u32::MAX);
        }
    }
    ...
    #[test]
    fn uszeu064_embeds_and_saturates() {
        assert_eq!(USZEU064.upper(u64::MAX), usize::MAX);
        // Lossless embed on the low range: ceil == `as`, and it round-trips.
        for v in [0usize, 1, 1_000, usize::MAX - 1, usize::MAX] {
            assert_eq!(USZEU064.ceil(v), v as u64);
            assert_eq!(USZEU064.upper(v as u64), v);
        }
    }

    #[test]
    fn uszeu128_embeds_and_saturates() {
        assert_eq!(USZEU128.upper(u128::MAX), usize::MAX);
        for v in [0usize, 1, 1_000, usize::MAX - 1, usize::MAX] {
            assert_eq!(USZEU128.ceil(v), v as u128);
            assert_eq!(USZEU128.upper(v as u128), v);
        }
    }
    ...
    proptest! {
        #[test]
        fn uszeu008_galois_l(a in arb_usize(), b in arb_u8()) {
            prop_assert!(conn_laws::galois_l(&USZEU008, a, b));
        }
        #[test]
        fn uszeu008_kernel_l(b in arb_u8()) {
            prop_assert!(conn_laws::kernel_l(&USZEU008, b));
        }
        #[test]
        fn uszeu016_galois_l(a in arb_usize(), b in arb_u16()) {
            prop_assert!(conn_laws::galois_l(&USZEU016, a, b));
        }
        #[test]
        fn uszeu016_kernel_l(b in arb_u16()) {
            prop_assert!(conn_laws::kernel_l(&USZEU016, b));
        }
        #[test]
        fn uszeu032_galois_l(a in arb_usize(), b in arb_u32()) {
            prop_assert!(conn_laws::galois_l(&USZEU032, a, b));
        }
        #[test]
        fn uszeu032_kernel_l(b in arb_u32()) {
            prop_assert!(conn_laws::kernel_l(&USZEU032, b));
        }
        #[test]
        fn uszeu064_galois_l(a in arb_usize(), b in arb_u64()) {
            prop_assert!(conn_laws::galois_l(&USZEU064, a, b));
        }
        #[test]
        fn uszeu064_kernel_l(b in arb_u64()) {
            prop_assert!(conn_laws::kernel_l(&USZEU064, b));
        }
        #[test]
        fn uszeu128_galois_l(a in arb_usize(), b in arb_u128()) {
            prop_assert!(conn_laws::galois_l(&USZEU128, a, b));
        }
        #[test]
        fn uszeu128_kernel_l(b in arb_u128()) {
            prop_assert!(conn_laws::kernel_l(&USZEU128, b));
        }
    }
}
