...
| `A123X456`           | `I008Q000`    | `i8` → `FixedI8<U0>` in `fixed::i8` (cross-crate iso)       |
...
**The crate's only top-level domain split is `core/` vs `fixed/`**
(post-Plan 26): every Conn whose endpoints all ship in Rust
`core`/`std` lives in `core/`; every Conn where at least one endpoint
is a [`fixed`](https://docs.rs/fixed)-crate `FixedI<N><F>` /
`FixedU<N><F>` wrapper lives in `fixed/`. The IEEE infrastructure
(`ExtendedFloat<T>`, ULP machinery, `BitsToF64Rd`, `RoundDownFromF64`,
`def_walk_helpers!`, `float_ext_int!` macros) stays in
`src/float.rs` as a type-and-helpers module analogous to
`src/extended.rs`; only the per-IEEE-width Conn submodules moved into
`core/`.
...
| `float`   | (this crate)                               | `ExtendedFloat<T>` + ULP/rounding helpers + `float_ext_int!` macros (no Conns; the per-IEEE-width Conns moved to `core/`) |
...
Per-host-type files in `core/X.rs` host §1 std-int Conns (generated
by `ext_int!`, `int_int_narrow!`, `int_uint!`, `int_uint_narrow!`,
`uint_int_sat!`, `uint_uint!`, `uint_uint_narrow!`) plus §3 NonZero
(via `nz_int_ext!` / `nz_uint_ext!`) plus the BE/LE byte-array isos
(`I008BE01`, `I008LE01`, …). Per-host-type files in `fixed/X.rs`
host §2 Q-format Conns (from per-host `fix_fix_iN!` / `fix_fix_uN!`
internal macros) plus §4 cross-crate isos (`I008Q000`, `U008Q000`,
etc.) plus float-bridge Conns (`F032Q###` / `F064Q###`, `F016Q###`
gated on `f16`). Cross-module name collisions (`Q008Q004` in both
`fixed::i008` and `fixed::u008`) are allowed and resolved by
qualified import.
...
| `F064F016`    | `f64` / `f16`                      | tie -> source wins            | `core::f064` (`f16` feature)    |
| `F032F016`    | `f32` / `f16`                      | tie -> source wins            | `core::f032` (`f16` feature)    |
| `I008I016`    | `Extended<i8>` / `i16`             | tie -> source wins            | `core::i008`                    |
| `U008I016`    | `Extended<u8>` / `i16`             | tie -> source wins            | `core::u008`                    |
...
| `I008N008`    | `i8` / `NonZeroI8`                 | both in core::num/std-int      | `core::i008`                    |
| `BOOLBE01`    | `bool` / `[u8; 1]`                 | (1) bool more specific         | `core::bool`                    |
