...
└── conn/
    ...
    └── std/
        ├── i8.rs       — Conns landing on i8 (I→I narrow + U→I non-widening)
        ├── i16.rs      — Conns landing on i16 (existing widening + narrow + U→I non-widening)
        ├── i32.rs      — Conns landing on i32
        ├── i64.rs      — Conns landing on i64
        ├── i128.rs     — Conns landing on i128 (incl. U128I128)
        ├── u8.rs       — Conns landing on u8 (existing I008U008 + U→U narrow + I→U narrow)
        ├── u16.rs      — Conns landing on u16
        ├── u32.rs      — Conns landing on u32
        ├── u64.rs      — Conns landing on u64
...
mirror APIs: `ceiling`/`upper` (L-side, takes `Cast 'L a b`) and
`floor`/`lower` (R-side, takes `Cast 'R a b`). The phantom `Side`
index `'L` / `'R` prevents
